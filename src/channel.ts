...
import { createChatChannelPlugin, type ChannelPlugin, type OpenClawConfig } from "openclaw/plugin-sdk/core";
import {
...
import { listVkAccountIds, resolveDefaultVkAccountId, resolveVkAccount, type ResolvedVkAccount } from "./accounts.js";
import { startVkGatewayAccount, notifyVkPairingApproval, sendVkTextFromRuntime } from "./channel.runtime.js";
...
const vkConfigAdapter = createScopedChannelConfigAdapter<ResolvedVkAccount>({
  ...
  formatAllowFrom: (allowFrom) =>
    allowFrom
      .map((entry) => String(entry).trim().replace(/^vk:/i, ""))
      .filter(Boolean),
});
